...
[Zeroconf](ZeroConfigurationNetworking(Zeroconf)http://www.zeroconf.org) is a collection of protocols including ipv4 link local
networking, [mDNS](MulticastDomainNameSystem(mDNS)http://www.multicastdns.org), and [DNS service-discovery](DNSservice-discovery(DNS-SD)http://www.dns-sd.org). This document aims
to outline an extension for BitTorrent clients to allow them to find
peers on a local-link network using the zeroconf [DNS
service-discovery](DNSservice-discovery(DNS-SD)http://www.dns-sd.org) system. It was written in the hope that clients
...
1. Apple's [Bonjour](Apple'sBonjourhttp://developer.apple.com/networking/bonjour/) for Mac OS X (included by default), Windows and POSIX systems
...
- The mainline client and others may like to examine [pybonjour](pybonjourhttp://o2s.csail.mit.edu/o2s-wiki/pybonjour). pybonjour provides a pure-Python interface to Apple [Bonjour](Apple'sBonjourhttp://developer.apple.com/networking/bonjour/) and compatible DNS-SD libraries (such as [Avahi](Avahihttp://www.avahi.org)).
...
<!-- Source: https://bittorrent.org/beps/bep_0026.html -->
