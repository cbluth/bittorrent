# BEP 37 — Anonymous BitTorrent over proxies
...
client to use a proxy, running on a 3rd party machine, or running over [Tor](http://www.tor.org).
...
in [uTorrent](www.utorrent.com) and [libtorrent](www.libtorrent.org).
...
advertised in the BitTorrent handshake (see [BEP20](bep_0020.html)). It's also commonly exchanged as
part of the extended handshake (see [BEP10](bep_0010.html)), where a free-form string
...
The BitTorrent specification ([BEP3](bep_0003.html)) indicates that the peer ID is persistent
...
and vice versa. (See [BEP10](bep_0010.html)).
...
if it is persistent (listen port).
...
kinds of correlations can be made as with the peer ID.
...
An adversary could use such technique by pinning a specific listen port
...
<!-- Source: https://bittorrent.org/beps/bep_0037.html -->
